...
4. Execution and verification: before every actual native spawn or follow-up, enforce the task contract, dependency/graph version, scoped boundary, bounded total attempts, deadline, lease duration, required lease, and task-wide no-progress stop. `task-claim` gets the lease; every accepted `task-record` carries and releases that attempt lease, so a later attempt requires a fresh claim. Only success or a stop condition terminates the task; progress resets no-progress. One pinned policy and atomic task state span strategies. The host accepts a requested model and returns its native agent ID; it does not accept a caller-supplied attested `session_key`. A local sticky `session_key` is correlation only. A deterministic verifier binds active requirements, real artifacts, and independent evidence. Aji reports the verified outcome; no conversational role is credited with execution, merging, acceptance, or artifact writing.

The Go CLI declares deterministic stage contracts; Codex performs the actual calls. Except for pure conversation, a task is routed actively. User stops, insertions, and edits increment the requirement and task graph versions while reusing the staff instance and local correlation key. Only affected nodes and descendants are cancelled or invalidated; stale graph versions, attempts, and late receipts are rejected. Rebuild occurs only after a whole-graph veto or task-identity change. A native collaboration spawn proves the requested-model dispatch and returns a native agent ID; effective-model attestation requires host exposure.
...
- Aji uses `gpt-5.6-terra` by default and falls back to `gpt-5.6-sol` when Terra is unavailable; Luna is never Aji's default. Execution nodes request task-appropriate models and may follow their declared availability chain only before generation and only for declared availability failures.
- `preflight_workers[].model` and `workers[].model` are requested model ids. General Staff is deterministic state, not a model worker. The native host returns a native agent ID for the spawned worker; requested model metadata alone does not attest the provider-effective model. Worker fallbacks are only for declared pre-generation availability failures; unapproved model ids are rejected before dispatch.
...
- Each worker has a local deterministic correlation `session_key`; it remains sticky for task state but is never host attestation. Model selection remains sticky after generation starts. No A/B test, quality retry, or post-generation model switch is allowed.
- A worker branch is not complete until native-agent identity, requested model, retry/failure kind, result/context handles and bytes, task-contract bytes, delegation reason, and independent artifact verification evidence are present. Route metadata, local correlation, `wuji dispatch`, and `codex exec` cannot prove the effective model ran or complete a graph. Generic interactive output and sandbox-blocked/unavailable evidence are rejected. Effective model, token, cache, and billing fields stay unavailable unless the native host exposes them independently.
