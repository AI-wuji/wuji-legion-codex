...
- Terra 仅属于阿极主脑，用于路由、合并、写入和完成判断，不得作为子代理模型。
- Luna 仅处理不依赖父上下文的机械提取、受限检索和验证；Sol 处理需要高推理的只读子代理任务。两者均固定一次模型、一次生成、无自动回退。
- 在 GPT 5.6 策略内，子代理仅允许 `gpt-5.6-sol` 或 `gpt-5.6-luna`；禁止 `gpt-5.4`、`gpt-5.4-mini` 及任何其他 GPT 子代理模型。用户显式选择 Grok 或其他非 GPT 供应商/模型时，不执行这项 GPT 策略。
...
- 除简单、自包含问答外，所有任务必须积极路由；当 `wuji route` 输出 `preflight_workers` 或 `workers` 时，当前 Codex 宿主必须以每项精确 `model`、`session_key` 和 `fallback_models` 创建原生只读子代理。`wuji dispatch`、`codex exec` 与路由 JSON 只声明/检查契约，均不算执行凭据；阿极仍是唯一合并者和写入者。
- 每个 worker 必须返回原生 agent 标识、请求模型、payload 哈希/字节、失败种类、结果句柄和阿极验收；token/cache/成本仅在宿主实际提供时记录，缺失不得伪造。没有宿主执行证据，不得把分支标记为完成。
- Sol、Terra、Luna 之间不得假设共享提示缓存。每个 worker 在任务开始时选定一次模型，并在自己的 `session_key` 内保持粘性。代码任务只有在同一查询生成的内容寻址上下文工件通过源文件哈希、实际字节数、至少 60% 检索锚点覆盖率、代码摘录和总重放成本门禁后，才允许委派一个独立 Sol 实现分支。宿主必须按 `stable_capability_prefix -> context_payload -> task_contract` 发送确定性 prompt；演示与写作默认留在阿极。GPT 5.6 子代理不允许生成前或生成后自动切换模型，也不得因质量付费重试。
