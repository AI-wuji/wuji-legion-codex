...
| 2 | 不偏离要求 | 用户要求什么就做什么。不擅改范围不加功能不自我发挥 |
...
## 跨对话上下文
Codex对话各自独立。但以下数据持久化跨对话共享:
- 项目代码文件(磁盘上) - 任何对话都能读写
- 错误DNA数据库 .wuji-errors/ERRORS.md - 对话A修的bug对话B自动检查
- 文件历史版本 .wuji-backups/ - 所有对话共享备份池
- E盘快照 E:\wuji-legion-backup\workspace\Hermes - 全量备份任意对话可恢复

新对话自动热身: 用户说继续项目->检查文件结构->加载错误DNA->检查备份->汇总项目状态。不丢失上下文、不重复修bug、不覆盖代码。
