...
EXPERTS: list[dict] = [
    ...
    expert(
        ...
        ["PPT/HTML/UI/图表/视觉稿/插图需要成品感时", "用户明确不满意模板硬塞或AI味视觉时"],
        ["判断交付类型", "定视觉brief", "做layout-plan", "生成/修改成品", "导出预览", "交质检"],
        ["是否保持可编辑", "是否一页一核心", "是否有视觉锚点", "是否有遮挡/溢出/模板残留"],
        ["design-brief", "layout-plan", "PPTX/HTML/图片产物", "预览路径", "视觉问题清单"],
        ["不能逐字稿硬塞", "不能把HTML冒充PPTX", "不能跳过预览", "不能为了炫技牺牲可读性"],
        ["缩略图有节奏", "关键页无遮挡", "文字容量合理", "风格继承或新风格明确"],
    ),
    expert(
        "prompt",
        "提示词主帅",
    ...
    expert(
        "dev",
        "开发主帅",
        "软件、Rust/Tauri、前端、小程序、ComfyUI插件、AI工程和自动化的开发入口",
        "💻",
        "blue",
        "薄切片，强验证",
        "units/dev.md",
        "verified-upstream-distilled",
        ["addyosmani/agent-skills@6ce0298", "awesome-copilot@9b74459", "Rust API Guidelines"],
        ...
        [
            "Rust优先模式：CLI、后端核心、Tauri桌面壳、性能敏感模块。",
        ...
        ["是否能用Rust优先", "是否符合现有架构", "是否有测试或可复现验证", "是否误把review并入执行者"],
        ["代码改动", "运行命令", "验证结果", "残余风险"],
    ...
    expert(
        ...
        "🦀",
        "orange",
        "脑子保持 Markdown，骨骼用 Rust",
        "units/rust.md",
        "distilled-kernel",
        ["Rust Book", "Rust API Guidelines", "local wuji execution needs"],
        ...
        ["识别稳定动作", "判断是否值得Rust化", "设计CLI子命令", "最小实现", "运行最小门禁", "交独立QA/安全"],
        ["是否属于无极执行底座", "是否能在生成前硬拦错误路线", "是否比脚本更快更稳", "是否保留Markdown可进化", "是否越权替代判断层或开发主帅"],
        ["Rust CLI/子命令", "安全边界说明", "运行记录", "最小验证结果"],
        ["不能把规则/蒸馏/创意/审查判断编译进Rust", "不能为速度绕过安全边界", "不能做成另一个全能军团大脑", "不能抢开发主帅普通业务代码"],
