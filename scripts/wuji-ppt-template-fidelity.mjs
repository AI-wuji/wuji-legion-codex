...
import { loadArtifactToolUtils, loadJSZip } from "./wuji-ppt-node-utils.mjs";
...
function usage() {
  return [
    "Usage:",
    "  node scripts/wuji-ppt-template-fidelity.mjs --workspace <dir> --final-pptx <deck.pptx> [options]",
    "",
    "Options:",
...
async function scanPptxPlaceholders(finalPptx) {
  const issues = [];
  const zip = await loadZipArchive(finalPptx);
  for (const entry of zipNames(zip).filter((name) => /^ppt\/slides\/slide\d+\.xml$/.test(name))) {
...
async function scanOverlays(starterLayoutDir, finalLayoutDir, defaultSlideSize) {
  const issues = [];
  const starterLayouts = await readLayouts(starterLayoutDir);
  const finalLayouts = await readLayouts(finalLayoutDir);
  const slideArea = defaultSlideSize.width * defaultSlideSize.height;
...
async function checkTemplateFidelity(options) {
  const artifactToolUtils = await loadArtifactToolUtils();
  const { DEFAULT_SLIDE_SIZE } = artifactToolUtils;
  ...
  if (starterLayoutDir && finalLayoutDir) {
    issues.push(...(await scanOverlays(starterLayoutDir, finalLayoutDir, DEFAULT_SLIDE_SIZE)));
  }
...
async function main() {
  const artifactToolUtils = await loadArtifactToolUtils();
  const { parseArgs, requireArg } = artifactToolUtils;
