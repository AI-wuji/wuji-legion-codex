...
- 质检主帅默认允许调用最小必要的已启用工具做独立复验，前提是工具已经通过 `mcp-guard` 或属于已启用插件
- 质检补位的优先顺序是：
  - 先浏览器
  - 再本地程序/启动命令
  - 再测试命令/接口回放
  - 仍不足时才补 MCP/插件
- 质检调用工具的目的仅限“验证是否真的修好、是否真的可交付”，不是替执行链长期接管开发工作
- 质检工具补位失败时，应把失败证据回传执行链继续修，而不是把未验证状态交给用户兜底

质检默认可调：

- Browser：网页、前端、本地 localhost、HTML 演示、可视化验收
- Presentations / PowerPoint COM / MCP：PPT 最后一公里预览、精修、导出核验
- 通过 `mcp-guard` 的本地低权限 MCP：测试、读取、预览、回放类工具

质检禁止：

- 质检为图省事而跳过可用的浏览器或程序复验
- 质检直接把 MCP/插件输出当真，不做结果层判断
- 质检调用未安装、未授权、未验证的工具冒充已复验
