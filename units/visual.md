...
```
① GPT/content.md 生成 slide-spec.json
   - 目标受众、演讲场景、核心信息
   - 每页一句话结论、讲稿要点、是否需要配图
    ↓
...
- 逐字稿、页标题、讲稿、JSON 和生成脚本必须写入 UTF-8 文件后再运行，不用终端一行命令直接塞中文
- PowerShell 生成中文文件时必须显式 `-Encoding UTF8`；Python/Node 脚本必须以 UTF-8 源文件保存
- 导出后必须抽检 `ppt/slides/slide*.xml` 或渲染预览，确认中文真实存在
- 如果看到“???”、空白模板、中文丢失或乱码，不得交付，必须回到内容写入层重做
